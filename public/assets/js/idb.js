...
let db;
// establish a connection to IndexedDB database called 'pizza_hunt' and set it it to version
const request = indexedDB.open('pizza_hunt', 1);

// this event will emit if the the database version changes (non-existant to version 1, v1 to v2, etc)
...
getAll.onsuccess = function() {
    // if there was date in indexedDb's store, let's send it to the api server
    if (getAll.result.length > 0) {
        fetch('/api/pizzas', {
            ...
            .then(serverResponse => {
                ...
                const transaction = db.transaction(['new_pizza'], 'readwrite');
                // access the the new_pizza object store
                const pizzaObjectStore = transaction.objectStore('new_pizza');
